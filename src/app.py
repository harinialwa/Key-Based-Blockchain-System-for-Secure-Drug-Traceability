...
def connect_Blockchain_drug(acc):
    ...
    contract_address="0xFCAa5Eda2Eb8fd7DbD42ebdDFA2312494060C833"
...
def connect_Blockchain_register(acc):
    ...
    contract_address="0xE2D82ec5Ae817EEF92C7c62531783E786c5f32d4"
    with open(artifact_path) as f:
        contract_json=json.load(f)
        contract_abi=contract_json['abi']
...
@app.route('/login')
def loginPage():
    if not session.get("tx_hash"):
        return render_template('login.html')
    else:
        data=None
        hash=session.get("tx_hash")
        wallet=session.get("wallet")
        #print("SESSION : ",session.get("tx_hash"))
        return render_template('login.html',data={'hash':hash,'wallet':wallet})
...
def registerUser():
    ...
    print("TRANSACTION : ",type(tx_hash))
    print("TRANSACTION : ",tx_hash.hex())
    rec=web3.eth.waitForTransactionReceipt(tx_hash)
    rc_status=dict(rec).get('status')
    if(rc_status==1):
        session['tx_hash']=tx_hash.hex()
        session['wallet']=walletaddr
